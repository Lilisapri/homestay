<section class="page-section bg-dark" id="home">
	<div class="container">
		<h2 class="text-center">Katalog Homestay</h2>
	...
	<div class="w-100">
		...
		$room = $conn->query("SELECT * FROM `room_list` order by rand() ");
			while($row = $room->fetch_assoc() ):
				...
				$row['price'] = (string)$row['price']; // Convert price to text
		?>
			<div class="card d-flex w-100 rounded-0 mb-3 package-item">
				...
				<div class="w-100 d-flex justify-content-between">
					<span class="rounded-0 btn btn-flat btn-sm btn-primary"><i class="fa fa-tag"></i> <?php echo $row['price'] ?></span> <!-- Price now as text -->
					<a href="./?page=view_room&id=<?php echo md5($row['id']) ?>" class="btn btn-sm btn-flat btn-warning">Lihat Detail Kamar</a>
				</div>
				</div>
			</div>
		<?php endwhile; ?>
